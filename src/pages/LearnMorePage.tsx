    ...
    <>
      ...
      <div className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-slate-200">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-3 flex items-center">
          <button
            onClick={() => navigate('/')}
            className="flex items-center gap-1.5 px-3 py-1.5 text-slate-600 hover:text-slate-850 hover:bg-slate-100 rounded-lg transition-all text-sm sm:text-base font-semibold"
          >
      ...
      {/* Hero Section */}
      <div 
        className="text-white py-12 sm:py-20 bg-cover bg-center bg-no-repeat relative"
        style={{
          backgroundImage: `linear-gradient(rgba(0, 0, 0, 0.75), rgba(0, 0, 0, 0.75)), url(${UC_METC_LOGO_URL})`
        }}
      >
        <div className="max-w-6xl mx-auto px-4 sm:px-6 text-center relative z-10">
          <h1 className="text-3xl sm:text-5xl md:text-6xl font-bold mb-4 sm:mb-6 leading-tight">About UC METC SILMS</h1>
          <p className="text-sm sm:text-lg md:text-2xl text-white/90 max-w-2xl mx-auto leading-relaxed">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 sm:py-20">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 items-center">
          <div>
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-slate-800 mb-4 sm:mb-6">What We Do</h2>
            <p className="text-sm sm:text-base md:text-lg text-slate-600 mb-4 text-justify leading-relaxed">
            ...
            <p className="text-sm sm:text-base md:text-lg text-slate-600 text-justify leading-relaxed">
          ...
          <div className="bg-gradient-to-br from-purple-100 to-green-100 rounded-2xl p-6 sm:p-12 flex items-center justify-center shadow-inner">
            <div className="text-center">
              <img src={COOP_LOGO_URL} alt="UC METC Logo" className="w-20 h-20 sm:w-32 sm:h-32 rounded-full mx-auto mb-4 sm:mb-6 shadow-lg border-2 border-white" />
              <h3 className="text-lg sm:text-2xl font-bold text-slate-800 leading-tight">UC METC SILMS</h3>
              <p className="text-xs sm:text-sm text-slate-600 mt-2 leading-snug">Sales, Inventory, Locker, and <br className="hidden sm:block"/> Membership System</p>
              <p className="text-[10px] sm:text-xs text-slate-500 mt-4 leading-tight uppercase tracking-wider font-semibold">UC METC Multipurpose<br className="hidden sm:block"/> Cooperative</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-slate-50 py-10 sm:py-16">
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-slate-800 mb-6 sm:mb-12 text-center">Why Students Love UC METC SILMS</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            {highlights.map((highlight, idx) => (
              <div key={idx} className="bg-white p-4 sm:p-6 rounded-xl shadow-sm border border-slate-100 flex items-center space-x-3 sm:space-x-4 hover:shadow-md transition-shadow">
                <CheckCircle2 className="text-green-600 flex-shrink-0 w-5 h-5 sm:w-6 sm:h-6" />
                <p className="text-sm sm:text-base font-semibold text-slate-800 leading-tight">{highlight}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10 sm:py-20">
        <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-slate-800 mb-6 sm:mb-12 text-center">What You Can Do</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-8">
          {features.map((feature, idx) => {
            ...
            return (
              <div
                key={idx}
                className="bg-white rounded-xl p-5 sm:p-8 border border-slate-200 hover:border-green-400 hover:shadow-lg transition-all duration-300"
              >
                <div className="bg-gradient-to-br from-green-100 to-purple-100 w-10 sm:w-14 h-10 sm:h-14 rounded-xl flex items-center justify-center mb-4">
                  <Icon className="w-5 h-5 sm:w-7 sm:h-7 text-green-600" />
                </div>
                <h3 className="text-base sm:text-xl font-bold text-slate-800 mb-2 sm:mb-3 leading-tight">{feature.title}</h3>
