...
import { ChevronLeft, Download, CheckCircle, Clock, XCircle, Eye, X } from 'lucide-react';
...
export const TransactionPage: React.FC = () => {
  ...
  const downloadReceiptAsImage = async () => {
    ...
    try {
      ...
      const originalFontSmooth = (receiptRef.current.style as any).fontSmooth;
      const originalWebkitFontSmoothing = (receiptRef.current.style as any).webkitFontSmoothing;
      const originalTextRendering = receiptRef.current.style.textRendering;
      const originalBackfaceVisibility = receiptRef.current.style.backfaceVisibility;
      const originalPerspective = receiptRef.current.style.perspective;

      // Hide the footer with buttons and X button for cleaner receipt image
      ...
      (receiptRef.current.style as any).fontSmooth = 'always';
      (receiptRef.current.style as any).webkitFontSmoothing = 'antialiased';
      receiptRef.current.style.textRendering = 'optimizeLegibility';
      receiptRef.current.style.backfaceVisibility = 'hidden';
      receiptRef.current.style.perspective = '1000px';
      ...
      // Wait longer for the layout to update and fonts to render properly
      await new Promise(resolve => setTimeout(resolve, 300));

      // Generate ultra-high-quality image using dom-to-image
      const scale = 4; // Even higher scale for maximum quality
      const dataUrl = await domtoimage.toPng(receiptRef.current, {
        ...
        filter: (node: any) => {
          // Exclude any problematic elements
          if (node.classList && node.classList.contains('animate-scale-in')) {
            return false;
          }
          return true;
        }
      ...
      (receiptRef.current.style as any).fontSmooth = originalFontSmooth;
      (receiptRef.current.style as any).webkitFontSmoothing = originalWebkitFontSmoothing;
      receiptRef.current.style.textRendering = originalTextRendering;
      receiptRef.current.style.backfaceVisibility = originalBackfaceVisibility;
      receiptRef.current.style.perspective = originalPerspective;
      if (contentArea) {
      ...
      // Hide action buttons after download for cleaner view
  ...
  const transactions = sales.map(sale => {
    // Debug logging
    console.log('Sale data:', { id: sale?.id, receipt_no: sale?.receipt_no, order_type: sale?.order_type });
    ...
    return {
      ...
      items: (sale?.items || []).map((item: any) => {
        const fullPrice = item?.fullPrice || item?.full_price;
        return {
          id: item?.id,
  ...
  const availableStatuses = ['all', 'completed', 'pending', 'cancelled', 'balance-due'];
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-purple-200 via-purple-300 to-purple-400 py-4 sm:py-8 px-4 animate-slide-in-right">
        ...
        <div className="bg-white rounded-2xl border border-slate-100/50 shadow-md p-4 sm:p-6 mb-6 sm:mb-8">
          ...
          <div className="flex flex-wrap gap-2 sm:gap-3">
            {availableStatuses.map((status) => (
              ...
              >
                {status === 'balance-due' ? 'Balance Due' : status.charAt(0).toUpperCase() + status.slice(1)}
              </button>
            ))}
          </div>
        </div>
      ...
      {/* Receipt Modal Portal - Renders outside Layout to be truly fixed */}
